...
def show_augment(
    ...
    plt.figtext(x=0.035, y=1, s="original", fontsize=text_size)
    plt.figtext(x=0.4, y=1, s="after augmentation", fontsize=text_size)
    plt.savefig(file)
    plt.show()


def show_augment_tensor(
    file,
    dataset,
    num_augments,
    num_instances,
    ...
    for img, _ in dataset:
        ...
        for _ in range(num_augments):
            ...
            plt.subplot(nrows, ncols, counter)
            plt.axis("off")
            imshow_tensor(img=augmenter(img), cmap=cmap)
        if counter ==  ncols*nrows:
            break
    if title:
        plt.figtext(x=0.5, y=1.05, s=title, fontsize=title_size, ha="center")
